...
app.get('/api/auth/users', (req, res) => {
  res.json(USERS.map(u => ({ id: u.id, name: u.name, role: u.role, branch: u.branch })));
});

app.post('/api/auth/login', (req, res) => {
  const user = USERS.find(u => u.id === req.body.userId);
  if (!user) return res.status(401).json({ error: 'unauthorized', message: 'Unknown user' });
...
app.patch('/api/service-requests/:srId', requireRole('RM', 'Operations'), (req, res) => {
...
app.post('/api/customers/:customerId/ai/summary', requireAiRole, (req, res) => {
...
app.post('/api/customers/:customerId/ai/next-best-action', requireAiRole, (req, res) => {
  const c = getCustomerOr404(req, res); if (!c) return;
...
app.post('/api/customers/:customerId/ai/next-best-action/:actionId/decision', requireAiRole, (req, res) => {
  const c = getCustomerOr404(req, res); if (!c) return;
...
app.post('/api/customers/:customerId/ai/chat', requireAiRole, (req, res) => {
  const c = getCustomerOr404(req, res); if (!c) return;
...
app.post('/api/customers/:customerId/ai/contact-plan', requireAiRole, (req, res) => {
  const c = getCustomerOr404(req, res); if (!c) return;
...
app.patch('/api/customers/:customerId/ai/contact-plan', requireAiRole, (req, res) => {
  const c = getCustomerOr404(req, res); if (!c) return;
...
app.listen(PORT, () => console.log(`Mock Customer 360 API listening on http://localhost:${PORT}/api`));
