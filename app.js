...
async function boot() {
  try {
    if (!window.Keycloak) {
      throw new Error('Keycloak library failed to load');
    }

    const kc = new window.Keycloak(window.KEYCLOAK_CONFIG);
    ...
    state.token = kc.token;

    // Keep the token fresh and update the application state.
    kc.onTokenExpired = async () => {
      try {
        await kc.updateToken(30);
        state.token = kc.token;
      } catch (e) {
        console.error('Unable to refresh Keycloak token', e);
        await kc.login();
      }
    };

    // Get the authenticated user's information from the Keycloak token.
    ...
    state.user = {
      ...
      role: getKeycloakRole(tokenParsed),
      branch: tokenParsed.branch || ''
    };

    state.view =
      state.user.role === 'Manager' ? 'dashboard' :
      state.user.role === 'Auditor' ? 'audit' :
      state.user.role === 'Operations' ? 'queue' :
      'search';
    ...
    console.error('Keycloak boot error:', e);
    state.error = e.message || 'Unable to authenticate with Keycloak';
  }

  render();
}

function getKeycloakRole(token) {
...
async function loginAs() {
  ...
  // This function is kept so existing HTML references do not break.
  window.location.href =
    window.KEYCLOAK_CONFIG.url +
    '/realms/' +
    window.KEYCLOAK_CONFIG.realm +
    '/protocol/openid-connect/auth' +
    '?client_id=' + encodeURIComponent(window.KEYCLOAK_CONFIG.clientId) +
